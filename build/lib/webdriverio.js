...
var WebdriverIO = function WebdriverIO(args, modifier) {
    ...
    var reject = function reject(err, onRejected) {
        ...
        var screenshotPath = options.screenshotPath[0] === '/' ? options.screenshotPath : _path2['default'].join(process.cwd(), options.screenshotPath);
        ...
        */
        ...
        if (typeof err.screenshot === 'string') {
            ...
            client.next(prototype.saveScreenshot, [_path2['default'].join(screenshotPath, fileName)], 'saveScreenshot');
        }
        this.logger.info('\tSaved screenshot: ' + fileName);
        this.emit('screenshot', {
            data: err.screenshot,
            filename: fileName,
            path: _path2['default'].join(screenshotPath, fileName)
        });
